...
-- ou exécutez-le via votre client PostgreSQL
  ...
  SET media = (
    SELECT jsonb_agg(
      jsonb_build_object(
        'url', url,
        'type', CASE 
          WHEN media_types IS NOT NULL AND array_length(media_types, 1) >= row_number() OVER (ORDER BY ordinality)
          THEN media_types[row_number() OVER (ORDER BY ordinality)]
          ELSE 'image'
        END,
        'alt', 'Média du post'
      )
    )
    FROM unnest(media_urls) WITH ORDINALITY AS t(url, ordinality)
  )
...
-- FIN DE LA MIGRATION
-- ========================================
-- Vérifiez que tout s'est bien passé en regardant les messages de notification
-- Vous devriez voir des statistiques sur le nombre de posts migrés
